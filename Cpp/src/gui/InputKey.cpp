...
InputKey::InputKey(sf::Vector2f pos, sf::Vector2f taille,string eventName,EventManager * event){
...
void InputKey::onKeyUp(Event & event){
    if(!_isActive)
        return;
    cout<<"je suis dans ONKeyUp mais je ne fonctionne pas"<<endl;
...
void InputKey::updateGraphic(){
    if(_isActive){
        ...
        _text.setString(_eventName + " : " + EventManager::keyToString(_event->getEventKey(_eventName)));
    }
