...
MenuOptionTouch::MenuOptionTouch(sf::Vector2i* s,EventManager& event):Menu(s),_event(event)
{
    ...
    addObs(new Bouton("Retour",boutonRetourSize,positionRetour,"Menu"));

    int i=0;
    for(EventConf::iterator it=_event.getListKey().begin();it!=_event.getListKey().end();++it){
        addObs(new InputKey(Vector2f(positionBouton.x,positionBouton.y+0.08*_window_size->y*i),boutonSize,it->first,&_event));
        i++;
    }
